...
from dataset import DDSM_dataset, VinDr_dataset, ThongNhat_dataset
import numpy as np
import itertools
...
def main(cfg):
    ...
    if cfg.data.dataset_name == 'VinDr':
    # if cfg.data.dataset_name == 'ThongNhat':
        test_dataset = VinDr_dataset(cfg.data.root, view_laterality=cfg.data.laterality,
                                          split='test', transform=train__valid_transform)

    if cfg.data.dataset_name == 'ThongNhat':
    # if cfg.data.dataset_name == 'VinDr':
        test_dataset = ThongNhat_dataset(cfg.data.root, view_laterality=cfg.data.laterality,
                                          split='test', transform=train__valid_transform)
