...
class MV_DEFEAT_ddsm(nn.Module):
    def __init__(self, cfg):
        ...
        self.model = timm.create_model(
            self.cfg.arch.model, 
            pretrained=self.cfg.arch.pretrained  # This will automatically use ImageNet weights when True
        )
    ...
    def DS_Combine_MultiView(self, alpha_views):
        """
        Combine evidence from multiple views using DS
        alpha_views: List of evidence from different views
        """
        combined_alpha = alpha_views[0]
        
        for i in range(1, len(alpha_views)):
            combined_alpha, _ = self.DS_Combin_two(combined_alpha, alpha_views[i])
    ...
    def forward(self, lcc, rcc, lmlo, rmlo):
        ...
        elif self.cfg.data.analysis == 'multi':
            out = self.DS_Combine_MultiView([out_lcc_alpha, out_rcc_alpha, out_lmlo_alpha, out_rmlo_alpha])
            out = nn.Softplus()(out)
            return out